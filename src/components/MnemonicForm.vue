...
export default {
  ...
  methods: {
    ...
    keyup: function(e) {
      ...
      if (value) {
        ...
        if (e.keyCode === 9) {
          ...
          if (
            this.suggestions.length < this.index.suggestion + 1 ||
            this.index.suggestion < 0
          ) {
            this.index.suggestion = 0;
          }
          ...
          const selected =
            (this.refs.suggestions || [])[this.index.suggestion] || false;
      ...
      if (e.keyCode === 9 && this.suggestions.length === 1) {
      } else if (
      ...
      if (!this.whitelist.includes(value)) {
        ...
        // Insert Word Using Active Suggestion Index
          ...
          mnemonic.splice(
            this.index.field,
            1,
            this.suggestions[this.index.suggestion] || this.suggestions[0]
          );
        }
      } else {
    ...
    set: function(word) {
      ...
      if (this.index.field >= this.mnemonic.length) {
...
.word--error .word-index {
  color: #ff627e;
}
.word--error .word-text,
.word--error .word-index {
  background: #fbf1f4;
  border-color: #ff627e;
}
...
.word-text:not(:focus) + .word-index {
  filter: blur(2px);
  transform: translate(12px, -50%) scale(0.8);
}
.word-text:focus,
.word-text:focus + .word-index {
  outline: none;
}
</style>
